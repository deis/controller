...
test-unit:
	cd rootfs \
		&& coverage run manage.py test --settings=deis.settings.testing --noinput registry api \
		&& coverage report -m

test-unit-quick:
	cd rootfs \
		&& ./manage.py test --settings=deis.settings.testing --noinput --parallel ${TEST_PROCS} --noinput registry api
